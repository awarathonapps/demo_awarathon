...
class Adoption extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();
        $acces_management = $this->check_rights('adoption');
    ...
    public function assessment_wise_manager()
    {
     ...
     public function assessment_division(){
        $assessment_html = '';
        $lastAssessmentId = ($this->input->post('assessmentid', TRUE) ? $this->input->post('assessmentid', TRUE) : 0);
        $assessment_list = $this->adoption_model->getdepartment($lastAssessmentId);
        $assessment_html .= '<option value="">';
        if (count((array)$assessment_list) > 0) {
            foreach ($assessment_list as $value) {
                $assessment_html .= '<option value="' . $value['user_id'] . '">[' . $value['user_id'] . '] - ' . $value['department'] . '</option>';
            }
        }
    ...
    public function assessment_started($returnflag = 0)
    {
        ...
        $indexStartGraph = $this->load->view('adoption/assessment_index_start', $Rdata, true);
    ...
    public function get_raps_mapped_user($returnflag = 0)
    {
        ...
        if ($IsCustom == '' or $IsCustom == "Current Year") {
            ...
            $report_xaxis_title = 'Yearly';
            $Day_type = 'current';
            $UserCount = $this->adoption_model->get_raps_mapped_user($StartDate, $EndDate, $Day_type, $Company_id);
            for ($i = 1; $i <= $current_month; $i++) {
                $day = str_pad($i, 2, "0", STR_PAD_LEFT);
                $TempDate = Date('Y-' . $day . '-01');
        ...
        $Rdata['count'] = json_encode($truefalse, JSON_NUMERIC_CHECK);
        $Raped_userGraph = $this->load->view('adoption/raps_mapped_user', $Rdata, true);
    ...
    public function assessment_complted($returnflag = 0)
    {
        ...
        $Rdata['count'] = json_encode($truefalse, JSON_NUMERIC_CHECK);
        $indexEndGraph = $this->load->view('adoption/assessment_index_end', $Rdata, true);
    ...
    public function total_video_uploded($returnflag = 0)
    {
        ...
        $total_video_uploaded = $this->load->view('adoption/total_video_uploaded', $Rdata, true);
    ...
    public function total_video_processed($returnflag = 0)
    {
        ...
        $total_video_processed = $this->load->view('adoption/total_video_processed', $Rdata, true);
    ...
    public function total_users_Ac_In($returnflag = 0)
    {
        ...
        $Airgraph = $this->load->view('adoption/AIr_grpah', $Rdata, true);
    ...
    public function get_raps_played_completed($returnflag = 0)
    {
        ...
        $Rdata['count'] = json_encode($truefalse, JSON_NUMERIC_CHECK);
        $Played_Raps_Completed = $this->load->view('adoption/PlayedCompletedGraph', $Rdata, true);
    ...
    public function total_report_sent($returnflag = 0)
    {
        ...
        if ($IsCustom == '' or $IsCustom == 'Current Year') {
            ...
            if ($type <= "15") {
                ...
                $report_xaxis_title = 'Yearly';
                $Day_type = "365_days";
                $total_reports_sent = $this->adoption_model->total_video_processed($SDate, $EDate, $Day_type, $Company_id);
                $begin = new DateTime($SDate);
                $end   = new DateTime($EDate);
                for ($i = $begin; $i <= $end; $i->modify('+1 month')) {
                    $day = $i->format("m-Y");
        ...
        $Rdata['completed_dataset'] = json_encode($completed_dataset, JSON_NUMERIC_CHECK);
        $Rdata['index_label'] = json_encode($index_label);
        $total_report_sent = $this->load->view('adoption/total_report_sent', $Rdata, true);
    ...
    public function adoption_by_team($returnflag = 0)
    {
        ...
        $TrainerIDSet = $this->input->post('manager_id', TRUE);
        $this->load->model('adoption_model');
        $report_data = array();
        $index_dataset = array();
        $report_title = '';
        ...
        if ($Assessment_id == '') {
            ...
            $lastAssessmentId = isset($LAssessmentDetails[0]['id']) ? $LAssessmentDetails[0]['id'] : '0';
            $report_title = isset($LAssessmentDetails[0]['assessment']) ? $LAssessmentDetails[0]['assessment'] : '';
            $FiveManager = $this->adoption_model->GetFiveManager($lastAssessmentId);
            $TrainerIDSet = array();
            if(!empty($FiveManager)){
                ...
                for ($i = 0; $i < count($getUserstart); $i++) {
                    ...
                    if (isset($FiveManager[$i]['trainer_name'])) {
                        $index_dataset[] = $FiveManager[$i]['trainer_name'];
                    } else {
                        $index_dataset[] = '';
                    }
                }
            ...
            $customTitle = $this->adoption_model->GetAssessmentName($Assessment_id);
            ...
            for ($i = 0; $i < count($getManagerName); $i++) {
                $index_dataset[] = isset($getManagerName[$i]['trainer_name']) ? $getManagerName[$i]['trainer_name']: "Empty Data" ;
                $Userstarted[] = isset($getUserstart[$i]['per_user_strated']) ? $getUserstart[$i]['per_user_strated'] : '0';
                $Usercompleted[] = isset($getUserstart[$i]['per_user_completed']) ? $getUserstart[$i]['per_user_completed'] : '0';
            }
        }
        ...
        $ad_byTeam = $this->load->view('adoption/AdoptionByTeam', $Rdata, true);
        $data['adoption_by_teams'] = $ad_byTeam;
        if ($returnflag) {
            return $data;
        } else {
            echo json_encode($data);
    ...
    public function adoption_by_divison($returnflag = 0)
    {
        ...
        if ($Assessment_id == '') {
            ...
            $Department = $this->adoption_model->getdepartment($lastAssessmentId);
            $DepartmentName = array();
            if(!empty($Department)){
                for ($l = 0; $l < count($Department); $l++) {
                    $DepartmentIdSet[] = $Department[$l]['user_id'];
                    $DepartmentName[] = $Department[$l]['department'];
                }
                $getUserstart = $this->adoption_model->GetUserDepartmentwise($lastAssessmentId, $DepartmentIdSet);
                for ($i = 0; $i < count($getUserstart); $i++) {
                    if (isset($DepartmentName[$i])) {
                        $index_dataset[] = $DepartmentName[$i];
                    } else {
                        $index_dataset[] = '';
                    }
                    $Userstarted[] = isset($getUserstart[$i]['per_user_strated']) ? $getUserstart[$i]['per_user_strated'] : '0';
            ...
            $customTitle = $this->adoption_model->GetAssessmentName($Assessment_id);
            $report_title = $customTitle[0]['assessment'];
            $GetDivisionName = $this->adoption_model->GetDivisionName($DivisionSet);
            $getUserstart = $this->adoption_model->GetUserDepartmentwise($Assessment_id, $DivisionSet);
            for ($i = 0; $i < count($GetDivisionName); $i++) {
        ...
        $Rdata['index_label'] = json_encode($Userstarted, JSON_NUMERIC_CHECK);
        $Rdata['user_completed'] = json_encode($Usercompleted, JSON_NUMERIC_CHECK);
        $ad_byDivision = $this->load->view('adoption/AdoptionByDivision', $Rdata, true);
    ...
    public function adoption_by_module($returnflag = 0)
    {
        ...
        if ($Assessment_id == '') {
            $CurrentDate = date("Y-m-d h:i:s");
            $LAssessmentDetails = $this->adoption_model->LastExpiredFiveAssessment($CurrentDate,$Company_id);
            for ($i = 0; $i < 5; $i++) {
            ...
            $getUserstart = $this->adoption_model->GetUserAssessmentwise($lastAssessmentId,$Company_id);
            for ($i = 0; $i < 5; $i++) {
                $Userstarted[] =    isset($getUserstart[$i]['per_user_strated']) ? $getUserstart[$i]['per_user_strated'] : '0';
                $Usercompleted[] =    isset($getUserstart[$i]['per_user_completed']) ? $getUserstart[$i]['per_user_completed'] : '0';
            }
            ...
            $getUserstart = $this->adoption_model->GetUserAssessmentwise($Assessment_id,$Company_id);
        ...
        $Rdata['index_label'] = json_encode($Userstarted, JSON_NUMERIC_CHECK);
        $Rdata['user_completed'] = json_encode($Usercompleted, JSON_NUMERIC_CHECK);
        $ad_byModule = $this->load->view('adoption/AdoptionByModule', $Rdata, true);
    ...
    public function adoption_by_region($returnflag = 0)
    {
        ...
        if ($Assessment_id == '') {
            ...
            $Getregion = $this->adoption_model->getregion_id($lastAssessmentId,$Company_id);
            $regionid = [];
            $loopcount = count($Getregion);
            for ($f = 0; $f < $loopcount; $f++) {
                $regionid[] = isset($Getregion[$f]['region_id']) ? $Getregion[$f]['region_id'] : '0';
            }
            if(!empty($regionid)){
                $getUserstart = $this->adoption_model->Adoption_by_region($lastAssessmentId,$Company_id,$regionid);
                $report_title=isset($getUserstart[0]['assessment']) ? $getUserstart[0]['assessment'] : 'Empty Data';
                $loopcount1 = count($regionid); 
                for ($i = 0; $i < $loopcount1; $i++) {
                    $lastAssessment[] =    isset($getUserstart[$i]['region_name']) ? $getUserstart[$i]['region_name'] : 'No Region';
                    $Userstarted[] =    isset($getUserstart[$i]['per_user_strated']) ? $getUserstart[$i]['per_user_strated'] : '0';
                    $Usercompleted[] =    isset($getUserstart[$i]['per_user_completed']) ? $getUserstart[$i]['per_user_completed'] : '0';
                }
            }
            ...
            if(!empty($Region_id)){
                $getUserstart = $this->adoption_model->Adoption_by_region($Assessment_id,$Company_id,$Region_id);
                $report_title=isset($getUserstart[0]['assessment']) ? $getUserstart[0]['assessment'] : 'Empty Data';
                $new = count($Region_id);  
                for ($i = 0; $i < $new; $i++) {
                    $lastAssessment[] =    isset($getUserstart[$i]['region_name']) ? $getUserstart[$i]['region_name'] : 'No Region';
        ...
        $data['report'] = $report_data;
        $Rdata['report_title'] = isset($report_title) ? json_encode($report_title) : '';
        $Rdata['index_dataset'] = json_encode($lastAssessment);
        $Rdata['index_label'] = json_encode($Userstarted, JSON_NUMERIC_CHECK);
        $Rdata['user_completed'] = json_encode($Usercompleted, JSON_NUMERIC_CHECK);
        $ad_byRegion = $this->load->view('adoption/AdoptionByRegion', $Rdata, true);
