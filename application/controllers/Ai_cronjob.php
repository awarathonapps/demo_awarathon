...
class Ai_cronjob extends CI_Controller {
    ...
    public function cronjob(){
        ...
        if (isset($all_assessment_result) AND count((array)$all_assessment_result)>0){
            foreach($all_assessment_result as $allassdata){
                ...
                $ass_users_result = $this->ai_cronjob_model->get_assessment_users($temp_company_id,$temp_assessment_id,$temp_assessment_type);
                ...
                if ($_process_started==0 OR $_process_started=="0"){
                    ...
                    if (isset($task_result) AND count((array)$task_result)>0){
                        foreach($task_result as $tdata){
                            ...
                            {
                                $audio_details = $this->ai_cronjob_model->audio_details($assessment_id,$question_id,$user_id);
                            ...
                            if ($db_task_id==""){
                                try {
                                    ...
                                    elseif($assessment_type==2){ //spotlight
                                        $json   = '{"portal_name":"'.$portal_name.'","assessment_name":"'.$assessment_name.'","person_name":"'.$user_name_id.'","question_number":"'.$question_series.'"}';
                                        $output = shell_exec(sprintf("python3.7 /var/www/html/awarathon.com/demo/spotlight_python/schedule_task_spotlight.py --data='".$json."' --audiotext='".$audio_totext."' --audio_id='".$audio_id."' 2>&1"));
                                    }
                                    elseif($assessment_type==3) //trinity
                                    {
                                        $json   = '{"portal_name":"'.$portal_name.'","assessment_name":"'.$assessment_name.'","person_name":"'.$user_name_id.'","question_number":"'.$question_series.'"}';
                                        $output = shell_exec(sprintf("source /var/www/html/awarathon.com/trinity/trinity/bin/activate"));
                                        $output = shell_exec(sprintf("/var/www/html/awarathon.com/trinity/trinity/bin/python /var/www/html/awarathon.com/trinity/rasa/schedule_task_trinity.py --data='".$json."' 2>&1"));
                                    }
                                    $log_data = [
                                        'py_parameters' => $json,
                                        'logs' => $output
                            ...
                            if (($task_status==0 OR ($task_status!==1 AND $task_status!==4)) AND  $company_id!="" AND $assessment_id!="" AND $user_id!="" AND $trans_id!="" AND $question_id!=""){
                                try {
                                    ...
                                    elseif($assessment_type==2){
                                        $output = shell_exec(sprintf("python3.7 /var/www/html/awarathon.com/demo/spotlight_python/task_status_spotlight.py --task_id='".$db_task_id."' 2>&1"));
                                    }
                                    elseif($assessment_type==3)
                                    {
                                        $output = shell_exec(sprintf("source /var/www/html/awarathon.com/trinity/trinity/bin/activate"));
                                        $output = shell_exec(sprintf("/var/www/html/awarathon.com/trinity/trinity/bin/python /var/www/html/awarathon.com/trinity/rasa/task_status_trinity.py --task_id='".$db_task_id."' 2>&1"));
                                    }
                                    $output = json_decode($output);
                                    if (isset($output) AND isset($output->success) AND $output->success=="true" AND isset($output->id)){
                            ...
                            if ($task_status==1 AND $xls_generated!==1){
                                if ($company_id!="" AND $assessment_id!="" AND $user_id!="" AND $trans_id!="" AND $question_id!=""){
                                    try {
                                        ...
                                        elseif($assessment_type==2){
                                            $output = shell_exec(sprintf("python3.7 /var/www/html/awarathon.com/demo/spotlight_python/download_file_spotlight.py --domain_name='".$portal_name."' --assessment_name='".$assessment_name."' --task_id='".$db_task_id."' 2>&1"));
                                        }
                                        elseif($assessment_type==3)
                                        {
                                            $output = shell_exec(sprintf("source /var/www/html/awarathon.com/trinity/trinity/bin/activate"));
                                            $output = shell_exec(sprintf("/var/www/html/awarathon.com/trinity/trinity/bin/python /var/www/html/awarathon.com/trinity/rasa/download_file_trinity.py --domain_name='".$portal_name."' --assessment_name='".$assessment_name."' --task_id='".$db_task_id."' 2>&1"));
                                            // var_dump($output);
                                        }
                                        $output = json_decode($output);
                                ...
                                if ($company_id!="" AND $assessment_id!="" AND $user_id!="" AND $trans_id!="" AND $question_id!=""){
                                    ...
                                    if (file_exists($temp_file_path)==TRUE){
                                        ...
                                        if ($worksheet_max_row <= 1) {
                                            ...
                                            //Jagdisha Patel: For mail : 16-02-2023 - Notification Module
                                            $send_notfication = 0;
