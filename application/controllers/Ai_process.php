...
class Ai_process extends MY_Controller
{
    ...
    public function fetch_participants_ai_dashboard()
    {
        $html = '';
        ...
        $_participants_result = $this->ai_process_model->get_distinct_participants($company_id, $asssessment_id);
    ...
    public function ideal_list($Encode_id)
    {
        ...
        $bestvideo = $this->common_model->get_value('assessment_mst', 'show_best_video', 'id=' . $data['assessment_id']);
        $data['is_bestvideo'] = isset($bestvideo->show_best_video) ? $bestvideo->show_best_video : 0;
    ...
    public function update_best_video()
    {
        ...
        $best_video = $this->input->post('best_video');
    ...
    function fetch_participants()
    {
        ...
        $company_id = $this->input->post('company_id', true);
        $asssessment_id = $this->input->post('assessment_id', true);
        $report_type_result = $this->common_model->get_value('assessment_mst', 'report_typem assessment_type', 'company_id="' . $company_id . '" AND id="' . $asssessment_id . '"');
        $report_type = 0;
        $assessment_type = 0;
        if (isset($report_type_result) and count((array) $report_type_result) > 0) {
            $report_type = (int) $report_type_result->report_type;
        ...
        $_participants_result = $this->ai_process_model->get_distinct_participants($company_id, $asssessment_id, $assessment_type);
        $data['report_type'] = $report_type;
        $data['_participants_result'] = $_participants_result;
    ...
    public function view_ai_process($_company_id, $_assessment_id, $_user_id)
    {
        ...
        if ($_company_id == "" or $_assessment_id == "" or $_user_id == "") {
            ...
            if (isset($overall_score_result) and count((array) $overall_score_result) > 0) {
                ...
                $istester = isset($overall_score_result->istester) ? $overall_score_result->istester : 0;
            }
            ...
            foreach ($question_result as $qr) {
                ...
                // $question_your_video_result = $this->ai_process_model->get_your_video($_company_id, $_assessment_id, $_user_id, $_trans_id, $question_id, $assessment_type);
                ...
                $question_best_video_result = $this->ai_process_model->get_best_video($_company_id, $_assessment_id, $question_id, $assessment_type);
            ...
            // echo "<pre>"; print_r($parameter_score_result);
    ...
    public function view_combine_reports($_company_id, $_assessment_id, $_user_id)
    {
        ...
        if ($_company_id == "" or $_assessment_id == "" or $_user_id == "") {
            ...
            // $this->load->library('Pdf_Library');
            echo             $htmlContent = $this->load->view('ai_process/combined_pdf', $data, true);
            exit;
            // //DIVEYSH PANCHAL
            ob_start();
            define('K_TCPDF_EXTERNAL_CONFIG', true);
    ...
    function generate_header()
    {
        ...
        $params = [];
        if (!empty($parameter_score_result) && ($report_type == 1 || $report_type == 3)) {
            foreach ($parameter_score_result as $psr) {
                $parameter_id = $psr->parameter_id;
                $parameter_name = $psr->parameter_name;
        ...
        $manual_params = [];
        if (!empty($parameter_score_manaul_result) && ($report_type == 2 || $report_type == 3)) {
    ...
    public function ajax_assessmentwise_data()
    {
        ...
        $assessment_list = $this->ai_process_model->get_participants_final_report_ai_reports($assessment_id);
    ...
    function generate_report()
    {
        ...
        if ($report_type == 1 || $report_type == 3) {
            ...
            if (!empty($ass_parameter_scores)) {
                foreach ($ass_parameter_scores as $parameter_score) {
                    ...
                    // $params[$parameter_score->parameter_label_id] = $parameter_score->parameter_name . ' AI %';
                }
            }
        }
        ...
        if ($report_type == 2 || $report_type == 3) {
            ...
            if (!empty($ass_manaul_parameter_score)) {
                foreach ($ass_manaul_parameter_score as $mps) {
                    ...
                    // $manaul_params[$mps->parameter_label_id] = $mps->parameter_name . ' Manual %';
                }
            }
        }
        // Ajit 02_09_2023 End
        ...
        foreach ($user_details as $ud) {
            ...
            $overall_score = 0;
            $ai_score1 = 0;
            if (!empty($user_ai_score) && isset($user_ai_score[$user_id]) && ($report_type == 1 || $report_type == 3)) {
            ...
            if (isset($manual_score) and count((array) $manual_score) > 0 && ($report_type == 2 || $report_type == 3)) {
    ...
    public function exportReport()
    {
        ...
        if ($Company_id != "") {
            ...
            $manualparmsColumns = $this->ai_process_model->get_manaul_parameters_reports($company_id, $assessment_id1);
            ...
            $params = [];
            if ($report_type == 1 || $report_type == 3) {
                $parameter_result = $this->ai_process_model->get_parameters_reports($company_id, $assessment_id1);
                if (!empty($parameter_result)) {
                    foreach ($parameter_result as $psr) {
                        $params[$psr->parameter_label_id] = $psr->parameter_name . ' AI %';
                    }
                }
                $ass_parameter_scores = $this->ai_process_model->assessment_get_parameter_sub_parameter_score($company_id, $assessment_id1);
                if (!empty($ass_parameter_scores)) {
                    foreach ($ass_parameter_scores as $parameter_score) {
                        $user_parameter_score[$parameter_score->user_id][$parameter_score->parameter_label_id] = [
                            'parameter_name' => $parameter_score->parameter_name,
            ...
            $manaul_params = [];
            if ($report_type == 2 || $report_type == 3) {
                $parameter_manaul_result = $this->ai_process_model->get_manaul_parameters_reports($company_id, $assessment_id1);
                if (!empty($parameter_manaul_result)) {
                    foreach ($parameter_manaul_result as $mpsr) {
                        $manaul_params[$mpsr->parameter_label_id] = $mpsr->parameter_name . ' Manual %';
                    }
                }
                $ass_manaul_parameter_score = $this->ai_process_model->get_manual_parameter_score($company_id, $assessment_id1);
                if (!empty($ass_manaul_parameter_score)) {
                    foreach ($ass_manaul_parameter_score as $mps) {
                        $user_manaul_parameter_score[$mps->user_id][$mps->parameter_label_id] = [
                            'parameter_name' => $mps->parameter_name,
            ...
            foreach ($user_details as $ud) {
                ...
                $user_list[$x]['AI Score'] = $ai_score1 != 0 ? $ai_score1 : '-';
                ...
                $user_list[$x]['Assessor Rating'] = $manual_overall_score != 0 ? $manual_overall_score : '-';
    ...
    { //In use for Export
        ...
        $Data_list = $this->ai_process_model->get_questions_user_details($c_id, $assessment_id, $user_id);
    ...
    public function export_trinity_dump()
    {
        ...
        // $transcripts_data = $this->ai_process_model->transcripts_data($assessment_id, $c_id, $user_id); // old data
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('E1', 'Added Date');
        ...
        if (!empty($full_transcript_details)) {
            ...
            foreach ($full_transcript_details as $v5) {
                ...
                $objPHPExcel->getActiveSheet()
                    ...
                    ->setCellValue("E$c", "$v5->addeddate");
                $objPHPExcel->getActiveSheet()->getStyle("A$a:E$a")->getFill();
            }
        } else {
            $c = 2;
            $objPHPExcel->getActiveSheet()
