...
class Ai_reports extends MY_Controller
{
    ...
    function generate_header()
    {
        ...
        $parameter_score_manaul_result = $this->ai_reports_model->get_manaul_parameters_reports($company_id, $assessment_id);
        $params = [];
    ...
    function generate_report()
    {
        ...
        foreach ($user_details as $ud) {
            ...
            $ass_manaul_parameter_score = $this->ai_reports_model->get_manual_parameter_score($company_id, $assessment_id, $user_id);
            $manaul_params = [];
            if (!empty($ass_manaul_parameter_score)) {
                foreach ($ass_manaul_parameter_score as $mps) {
                    // $parameter_id = $mps->parameter_id;
                    $mparameter_name = $mps->parameter_name . ' Manual %';
                    $manaul_params[] = $mparameter_name;
                    // $parameter_label_id = $mps->parameter_label_id;
                    $user_list[$x][$mparameter_name] = !empty($mps->score) ? $mps->score : '-';
                    // $muser_score[] = !empty($mps->score) ? $mps->score : '-';
                }
            }
        ...
        // Ajit 10-07-2023 - Start
        $parameter_score_manaul_result = $this->ai_reports_model->get_manaul_parameters_reports($company_id, $assessment_id1);
        if (!empty($parameter_score_manaul_result)) {
            foreach ($parameter_score_manaul_result as $mpsr) {
                $dtDisplayColumns[$y] = $mpsr->parameter_name . ' Manual %';
                $y++;
            }
        }
        // Ajit 10-07-2023 - End
    ...
    public function exportReport()
    {
        ...
        if ($Company_id != "") {
            ...
            $manualparmsColumns = $this->ai_reports_model->get_manaul_parameters_reports($company_id, $assessment_id1);
            ...
            foreach ($user_details as $ud) {
                ...
                if (!empty($ass_manaul_parameter_score)) {
                    foreach ($ass_manaul_parameter_score as $mps) {
                        ...
                        $manaul_params[$parameter_name] = !empty($mps->score) ? $mps->score : '-';
                    }
                }

                if (count((array) $manualparmsColumns) > 0) {
                    foreach ($manualparmsColumns as $column) {
                        // $parameter_id = $column->parameter_id;
                        $parameter_name = $column->parameter_name . ' Manual %';
                        $mparams[] = $parameter_name;
                        // $parameter_label_id = $column->parameter_label_id;
                        $user_list[$x][$parameter_name] = isset($manaul_params[$parameter_name]) ? $manaul_params[$parameter_name] : '-';
                        $user_manaul_score[] = isset($manaul_params[$parameter_name]) ? $manaul_params[$parameter_name] : '-';
                    }
                }
