...
class Assessment_dashboard extends MY_Controller {
    ...
    public function index() {
       ...
       //$assessment_list= $this->assessment_dashboard_model->get_assessment_list($company_id, $trainer_id, $start_date, $end_date);
        ...
        //----------------
        $data['assessment_data'] = $this->assessment_dashboard_model->get_assessment($data['company_id'], '', $start_date, $end_date, $supervisor_id);
    ...
    public function getdashboardData() {
        ...
        $data['Total_Assessment'] = $this->assessment_dashboard_model->get_Total_Assessment($Company_id, $SDate, $EDate, $region_id, $store_id, $supervisor_id, $report_type);
        $data['Candidate_Count'] = $this->assessment_dashboard_model->get_Candidate_Assessment($Company_id, $question_type, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $supervisor_id);
        $data['Avg_Accuracy'] = $this->assessment_dashboard_model->get_Average_Accuracy($Company_id,$report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $supervisor_id, $assessment_id1);
        $TopFiveParameter = $this->assessment_dashboard_model->get_top_five_parameter($Company_id, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $supervisor_id);
      ...
      //--- High and Low Accuracy--//
        ...
        if(count((array)$TopFiveParameter) > 0){
            ...
            $minSet = $this->assessment_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $existid, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $supervisor_id);
        ...
        $BottomFiveParameter = $this->assessment_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $top_five_para_id, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $supervisor_id);
        ...
        if (count((array)$session_set) > 0) {
            ...
            if($report_by==1){
                $region_data = $this->assessment_dashboard_model->overall_result_parameter(1,$passrange_from,$passrange_to,$failrange_from,$failrange_to,$SDate,$EDate,'','','',$store_id, $report_type);
            }else{
                $region_data = $this->assessment_dashboard_model->overall_result_assessment(1,$passrange_from,$passrange_to,$failrange_from,$failrange_to,$SDate,$EDate,'','','',$store_id,  $report_type);
            ...
            $AssessmentData = $this->assessment_dashboard_model->get_assessment($Company_id, '', $SDate, $EDate, $supervisor_id);
...
//        }
        ...
        if (count((array)$session_set) > 0) {
            ...
            $failrange_to =$session_set['Fail']['range_to'];
            if($report_by==1){
                $region_data = $this->assessment_dashboard_model->overall_result_parameter(0,$passrange_from,$passrange_to,$failrange_from,$failrange_to,$StartDate,$EndDate,$region_id,$assessment_id,$step,$store_id, $report_type);
            }else{
                $region_data = $this->assessment_dashboard_model->overall_result_assessment(0,$passrange_from,$passrange_to,$failrange_from,$failrange_to,$StartDate,$EndDate,$region_id,$assessment_id,$step,$store_id, $report_type);
                //echo "<pre>";
                //print_r($region_data);
            }
            if (isset($region_data['region_data']) AND count((array)$region_data['region_data']) > 0) {
...
//Added
      ...
      //  print_r($_POST);
        ...
        $regionset = $this->assessment_dashboard_model->get_region_result($Company_id, $report_by,$region_id,$StartDate,$EndDate,$assessment_id, $store_id, $report_type, $supervisor_id);
        //echo "<pre>";
        //print_r($regionset);
        $last_avg = array();
        ...
        if (count((array)$regionset) > 0) {
			if($report_by==1){
                $Avgset = $this->assessment_dashboard_model->get_region_average($Company_id, 1,$region_id,$StartDate,$EndDate,$assessment_id, $report_type ,$store_id);
				$Avgset2 = $this->assessment_dashboard_model->get_region_average($Company_id, 3,$region_id,$StartDate,$EndDate,$assessment_id, $report_type,$store_id);
			}else{
				$Avgset = $this->assessment_dashboard_model->get_region_average($Company_id, 2,$region_id,$StartDate,$EndDate,$assessment_id, $report_type,$store_id);
				$Avgset2 = $this->assessment_dashboard_model->get_region_average($Company_id, 4,$region_id,$StartDate,$EndDate,$assessment_id, $report_type,$store_id);
			}
    ...
    public function getDatewiseAssessment() {
        ...
        $data['company_id'] = $this->mw_session['company_id'];
        $assessmentchtml = '<option value="">All Assessment</option>';
        $AssessmentData = $this->assessment_dashboard_model->get_assessment($data['company_id'], '', $StartDate, $EndDate, $supervisor_id);
    ...
    public function parameter_scoredata() {
        ...
        $EDate = date('Y-m-d', strtotime($end_date));
        //print_r($_POST);
        $data['assessment_result'] = $this->assessment_dashboard_model->get_parameter_assessment_score($data['company_id'], $data['parameter_id'], $data['region_id'],$SDate,$EDate,$data['report_type'],$data['store_id']);        
    ...
    public function regionwise_table() {
      ...
      //  $result_data = $this->assessment_dashboard_model->get_parameter_user_result($company_id, $region_id, $assessment_id, $store_id);
        $result_data = $this->assessment_dashboard_model->get_parameter_user_result_new($company_id, $region_id, $assessment_id, $report_type);
        $Horizontal_avg =array();
        $Vertical_avg =array();
        $Final_avg=0;
        if (count((array)$result_data) > 0) {
               ...
               // $Avgset = $this->assessment_dashboard_model->get_user_average($company_id, 1,$region_id,$assessment_id,$store_id);
                  $Avgset = $this->assessment_dashboard_model->get_user_average($company_id, 1,$region_id,$assessment_id,$report_type);
                $Avgset2 = $this->assessment_dashboard_model->get_user_average($company_id, 3,$region_id,$assessment_id,$report_type);
    ...
    public function assessment_parameterwise_table() {
        ...
        $region_list = array();
        $regiondata_result = array();
        $result_data = $this->assessment_dashboard_model->get_parameter_assessment_result($company_id, $assessment_id, $report_type);
        ...
        if (count((array)$result_data) > 0) {
            ...
            $Avgset = $this->assessment_dashboard_model->get_pararegion_average($company_id,1,$assessment_id, $report_type);
            $Avgset2 = $this->assessment_dashboard_model->get_pararegion_average($company_id,3,$assessment_id, $report_type);
                foreach ($Avgset2 as $key => $rl) {
                        $Horizontal_avg[$rl->region_id] =$rl->result;
                }
                foreach ($Avgset as $key => $rl) {
    ...
    public function QuestionScoreDatatable() {
        ...
        $DTRenderArray = $this->assessment_dashboard_model->get_questions_score_new($dtWhere, $dtOrder, $dtLimit, $report_type);
        $output = array(
    ...
    public function region_level_scoredata(){
        ...
        if (count((array)$session_set) > 0) {
            ...
            $Result_data = $this->assessment_dashboard_model->region_level_result($region_id,$StartDate,$EndDate,$pass_range_from,$pass_range_to,$fail_range_from,$fail_range_to, $report_type);
          //  print_r($Result_data);  
			$pass_color_code = $this->session->userdata('Assessment_result_session')['Pass']['range_color'];                    
			$fail_color_code= $this->session->userdata('Assessment_result_session')['Fail']['range_color'];
			$pending_color_code= $this->session->userdata('Assessment_result_session')['Pending']['range_color'];
       ...
       $Company_id = $this->mw_session['company_id'];
        ...
        if ($rpt_period == "weekly") {
            if ($WeekStartDate != '' && $WeekEndDate != '') {
                ...
                $AccuracySet = $this->assessment_dashboard_model->assessment_index_weekly_monthly($report_by,$WeekStartDate, $WeekEndDate, $report_type,$parameter_id);
               ...
               $AccuracySet = $this->assessment_dashboard_model->assessment_index_weekly_monthly($report_by,$WeekStartDate, $WeekEndDate, $report_type);
            ...
            $AccuracySet = $this->assessment_dashboard_model->assessment_index_yearly($report_by,$WeekStartDate, $WeekEndDate, $report_type);
            //print_r($AccuracySet);
            for ($i = 1; $i <= 12; $i++) {
                $day = str_pad($i, 2, "0", STR_PAD_LEFT);
	...
	public function load_parameter_index($returnflag=0) {
       ...
       $Company_id = $this->mw_session['company_id'];
        ...
        $indexAssessGraph = $this->load->view('assessment_dashboard/parameter_index_report', $PRdata, true);
    ...
    public function ajax_assessmentwise_data() {
        ...
        $assessment_list= $this->assessment_dashboard_model->get_assessment($company_id,'', $start_date, $end_date, $userid_id);
        
        
        //print_r(count($assessment_list));
        //exit;
        if(count((array)$assessment_list)>0){
            foreach ($assessment_list as $value) {
              //  $assessment_html .='<option value="'.$value->id.'">'.$value->assessment.'</option>';
