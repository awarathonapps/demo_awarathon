...
class Assessment_trainee_dashboard extends MY_Controller {

    public function __construct() {
        parent::__construct();
        $acces_management = $this->check_rights('assessment_trainee_dashboard');
    ...
    public function index() {
        ...
        $data['trainee'] = $this->assessment_trainee_dashboard_model->get_user($data['company_id']);
        ...
        $data['trainee_data'] = $this->assessment_trainee_dashboard_model->get_Trainee_data($data['company_id'],$manager_id);
        // $data['trainee_data'] = $this->assessment_trainee_dashboard_model->get_Trainee($data['company_id'],$start_date,$end_date);
        //$data['parameter_data'] = $this->assessment_trainee_dashboard_model->get_parameter($data['company_id']);
        ...
        $this->load->view('assessment_trainee_dashboard/index', $data);
    }
    ...
    public function getdashboardData() {
        ...
        $data['Total_Assessment'] = $this->assessment_trainee_dashboard_model->get_Total_Assessment($Company_id, $SDate, $EDate, $user_id, $report_type);
        $data['question_answer'] = $this->assessment_trainee_dashboard_model->get_Total_Questions_Time($Company_id, $SDate, $EDate, $user_id, $report_type);
        //$data['Total_Questions'] = $Questions_Time['TotalQ'];
        //$data['Total_Time'] = $Questions_Time['TotalT'];
        $time= $this->assessment_trainee_dashboard_model->get_time($Company_id, $SDate, $EDate, $user_id);
       //print_r($time);
        $data['Total_Time'] =$time->total_time;
        $data['Avg_Accuracy'] = $this->assessment_trainee_dashboard_model->get_Average_Accuracy($Company_id,$report_by, $SDate, $EDate, $user_id, $report_type);
//        
        $TopFiveParameter = $this->assessment_trainee_dashboard_model->get_top_five_parameter($Company_id, $report_by, $SDate, $EDate, $user_id, $report_type);
        ...
        if(count((array)$TopFiveParameter) > 0){
            ...
            $minSet = $this->assessment_trainee_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $existid, $SDate, $EDate, $user_id, $report_type);
        ...
        $BottomFiveParameter = $this->assessment_trainee_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $top_five_para_id, $SDate, $EDate, $user_id, $report_type);
        ...
        //$parameter_data = $this->assessment_trainee_dashboard_model->get_parameter($Company_id,$user_id);
        $parameter_data = $this->assessment_trainee_dashboard_model->get_assessment($user_id, $SDate, $EDate, $report_type);
       ...
       $Company_id = $this->mw_session['company_id'];
        ...
        if ($rpt_period == "weekly") {
            if ($WeekStartDate != '' && $WeekEndDate != '') {
                $AccuracySet = $this->assessment_trainee_dashboard_model->assessment_index_weekly_monthly($report_by,$WeekStartDate, $WeekEndDate, $parameter_id, $user_id, $report_type);
                ...
                $EndWeek = date('d');
                $AccuracySet = $this->assessment_trainee_dashboard_model->assessment_index_weekly_monthly($report_by,$WeekStartDate, $WeekEndDate,  $user_id, $report_type);
            ...
            $AccuracySet = $this->assessment_trainee_dashboard_model->assessment_index_yearly($report_by,$WeekStartDate, $WeekEndDate, $user_id, $report_type);
            for ($i = 1; $i <= 12; $i++) {
                $day = str_pad($i, 2, "0", STR_PAD_LEFT);
	...
	public function load_parameter_index($returnflag=0) {
       ...
       $Company_id = $this->mw_session['company_id'];
        ...
        if ($rpt_period == "weekly") {
            if ($WeekStartDate != '' && $WeekEndDate != '') {
                $AccuracySet = $this->assessment_trainee_dashboard_model->parameter_index_charts_new($parameter_id, $report_by,$WeekStartDate, $WeekEndDate,  $user_id, $report_type);
                for ($i = $WeekStartDay; $i <= $WeekEndDay; $i++) {
                    $day = str_pad($i, 2, "0", STR_PAD_LEFT);
                    if ($Year != '' && $Month != '') {
                        $TempDate = $Year . '-' . $Month . '-' . $i;
