...
class Competency extends MY_Controller
{
    ...
    public function Competency_understanding_graph($returnflag = 0)
    {
        ...
        if ($Assessment_id == "") {
            ...
            $getname_id_type = $this->Competency_model->LastExpiredAssessment($StartStrDt, $EndDtdate);
            if (isset($getname_id_type) && !empty($getname_id_type)) {
                ...
                $getscore = $this->Competency_model->getCompetencyscore($assessment_id);
                $count  = '0';
                $count1 = '0';
                $count2 = '0';
                $count3 = '0';
    ...
    public function performance_comparison($returnflag = 0)
    {
        ...
        if ($Assessment_id == "") {
            ...
            $EndDtdate = date('Y-m-d');
            // $CurrentDate =  date("Y-m-d h:i:s");
            $getassessment = $this->Competency_model->LastExpiredFiveAssessment($StartStrDt, $EndDtdate);
    ...
    public function competency_by_division($returnflag = 0)
    {
        ...
        if ($Assessment_id == "" && $division_id_set == "") {
            if ($IsCustom == "") {
                ...
                $CurrentDate = date("Y-m-d");
                $last_ass = $this->Competency_model->get_assessment($start_date, $CurrentDate);
            } else if ($IsCustom == "Current Year") {
                $startdate = date('Y-01-01');
                $CurrentDate = date("Y-m-d");
                $last_ass = $this->Competency_model->get_assessment($startdate, $CurrentDate);
            } else {
                $last_ass = $this->Competency_model->get_assessment($SDate, $EDate);
            }
                ...
                if (!empty($division_details)) {
                    ...
                    if (count($division_set) != 0) {
                        ...
                        if ($IsCustom == "") {
                            $start_date = date('Y-m-d', strtotime("-30 days"));
                            $CurrentDate = date("Y-m-d");
                            $Get_divison_score = $this->Competency_model->get_division_data($start_date, $CurrentDate, $assessment_id, $division_set, $reg_id_set, $manager_id_set, $Company_id);
                        } else if ($IsCustom == "Current Year") {
                            $startdate = date('Y-01-01');
                            $CurrentDate = date("Y-m-d");
                            $Get_divison_score = $this->Competency_model->get_division_data($startdate, $CurrentDate, $assessment_id, $division_set, $reg_id_set, $manager_id_set, $Company_id);
                        } else {
                            $Get_divison_score = $this->Competency_model->get_division_data($SDate, $EDate, $assessment_id, $division_set, $reg_id_set, $manager_id_set, $Company_id);
                        }
                ...
                if (!empty($region_details)) {
                    ...
                    if (count($region_id) != 0) {
                        ...
                        if ($IsCustom == "") {
                            $start_date = date('Y-m-d', strtotime("-30 days"));
                            $CurrentDate = date("Y-m-d");
                            $get_region_score = $this->Competency_model->get_region_data($start_date, $CurrentDate, $assessment_id, $region_id, $manager_id_set, $Company_id);
                        } else if ($IsCustom == "Current Year") {
                            $startdate = date('Y-01-01');
                            $CurrentDate = date("Y-m-d");
                            $get_region_score = $this->Competency_model->get_region_data($startdate, $CurrentDate, $assessment_id,  $region_id, $manager_id_set, $Company_id);
                        } else {
                            $get_region_score = $this->Competency_model->get_region_data($SDate, $EDate, $assessment_id, $region_id, $manager_id_set, $Company_id);
                        }
    ...
    public function am_wise_region()
    {
        ...
        $Company_id = $this->mw_session['company_id'];
        $assessmentid =  $this->input->post('assessmentid', TRUE) != '' ? $this->input->post('assessmentid', TRUE) : '0';
        $assessment_list = $this->Competency_model->am_wise_region($assessmentid, $Company_id);
        if (count((array)$assessment_list) > 0) {
            foreach ($assessment_list as $value) {
    ...
    public function region_wise_performance($returnflag = 0)
    {
        ...
        if (isset($assessment_id) && !empty($assessment_id) && isset($region_id) && !empty($region_id)) {
            $region_wise_score = $this->Competency_model->get_region_score($assessment_id, $region_id, $above_range, $second_range_from, $second_range_to, $third_range_from, $third_range_to, $less_range);
            ...
            $start_date =  date('Y-m-d', strtotime("-30 days"));
            $CurrentDate =  date("Y-m-d");
            $Lastexassessment = $this->Competency_model->lastassessment($start_date, $CurrentDate);
    ...
    public function get_rockstars_user_score()
    {
        ...
        if ($assessment_id == "") {
            ...
            $end_date = date("Y-m-d");
            $get_assessment_id = $this->Competency_model->all_assessment($start_date, $end_date);
        ...
        if ($assessment_id != "") {
            ...
            if (!empty($user_final_scores)) {
                ...
                foreach ($user_final_scores as $ud) {
                    ...
                    $user_ai_score = $this->Competency_model->get_ai_score($am_id, $amuser_id);
                    ...
                    $user_manual_score = $this->Competency_model->get_manual_score($am_id, $amuser_id);
    ...
    public function get_manager_data()
    {
        ...
        if ($assessment_id == '' && $manager_id == '') {
            ...
            if ($is_custom == "") {
                ...
                $last_assessment_array = $this->Competency_model->get_assessment($start_date, $current_date);
            } else if ($is_custom == "Current Year") {
                $startdate = date('Y-01-01');
                $CurrentDate = date("Y-m-d");
                $last_assessment_array = $this->Competency_model->get_assessment($startdate, $CurrentDate);
            } else {
                $last_assessment_array = $this->Competency_model->get_assessment($SDate, $EDate);
            }
            if (count((array)$last_assessment_array) > 0) {
                foreach ($last_assessment_array as $k => $rld) {
                    $assessmentid[] = $rld->id;
                }
                $manager_details = $this->Competency_model->get_assessment_wise_manager($company_id, $assessmentid);
        ...
        if (isset($new_manager_id) && !empty($new_manager_id)) {
            ...
            for ($i = 0; $i < count((array)$new_manager_id); $i++) {
                ...
                if (isset($new_manager_id[$i]) && $new_manager_id[$i] != 0 && isset($new_avg_accuracy[$new_manager_id[$i]]) && $new_avg_accuracy[$new_manager_id[$i]] != '') {
