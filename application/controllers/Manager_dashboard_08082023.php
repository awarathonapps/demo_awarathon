...
class Manager_dashboard extends MY_Controller {
    ...
    public function index() {
       ...
       //$assessment_list= $this->assessment_dashboard_model->get_assessment_list($company_id, $trainer_id, $start_date, $end_date);
        ...
        $data['parameter_data'] = $this->manager_dashboard_model->get_parameter();
    ...
    public function getdashboardData() {
       ...
       // print_r($SDate);
        ...
        $data['Total_Assessment'] = $this->manager_dashboard_model->get_Total_Assessment($Company_id, $SDate, $EDate, $region_id, $store_id, $supervisor_id, $report_type);
        // print_r($data['Total_Assessment']);die;
        $data['Candidate_Count'] = $this->manager_dashboard_model->get_Candidate_Assessment($Company_id, $question_type, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $supervisor_id);
...
//        }
	...
	public function load_parameter_index($returnflag=0) {
       ...
       $Company_id = $this->mw_session['company_id'];
        ...
        if ($rpt_period == "weekly") {
            if ($WeekStartDate != '' && $WeekEndDate != '') {
                $AccuracySet = $this->assessment_trainee_dashboard_model->parameter_index_charts($parameter_id, $report_by,$report_type,$WeekStartDate, $WeekEndDate);
                for ($i = $WeekStartDay; $i <= $WeekEndDay; $i++) {
                    $day = str_pad($i, 2, "0", STR_PAD_LEFT);
                    if ($Year != '' && $Month != '') {
                        $TempDate = $Year . '-' . $Month . '-' . $i;
