...
class Parameterlabel extends MY_Controller {
    public function __construct() {
        parent::__construct();
        $acces_management = $this->check_rights('parameterlabel');
        if (!$acces_management->allow_access) {
            redirect('dashboard');
        }
        $this->acces_management = $acces_management;
        $this->load->model('parameterlabel_model');
    }        
    public function index() {
        ...
        $this->load->view('parameterlabel/index', $data);
    }
    public function create() {
        ...
        $this->load->view('parameterlabel/create', $data);
    }
    public function edit($id) {
        ...
        $this->load->view('parameterlabel/edit', $data);
    }   
    public function view($id) {
        ...
        $this->load->view('parameterlabel/view', $data);
    }
    public function DatatableRefresh() {
        ...
        $DTRenderArray = $this->parameterlabel_model->LoadDataTable($dtWhere, $dtOrder, $dtLimit);
        $output = array(
                ...
                if ($dtDisplayColumns[$i] == "status") {
                    ...
                    if ($acces_management->allow_add OR $acces_management->allow_view OR $acces_management->allow_edit OR $acces_management->allow_delete) {
                        ...
                        if ($acces_management->allow_view) {
                            $action .= '<li>
                                        <a href="' . $site_url . 'parameterlabel/view/' . base64_encode($dtRow['id']) . '">
                        ...
                        if ($acces_management->allow_edit) {
                            $action .= '<li>
                                        <a href="' . $site_url . 'parameterlabel/edit/' . base64_encode($dtRow['id']) . '">
    ...
    public function submit() {        
        ...
        if (!$acces_management->allow_add) {
            ...
            if ($SuccessFlag) {
                ...
                $insert_id = $this->common_model->insert('parameter_label_mst', $data);
    ...
    public function update($Encode_id) {
        ...
        if (!$acces_management->allow_edit) {
            ...
            if ($SuccessFlag) {
                ...
                $insert_id = $this->common_model->update('parameter_label_mst','id',$id,$data);
    ...
    public function remove() {
        ...
        if (!$acces_management->allow_delete) {
            ...
            $DeleteFlag = 1;
            if ($DeleteFlag) {                
                $this->common_model->delete('parameter_label_mst', 'id', base64_decode($deleted_id));                
        ...
        if ($Action == 1) {
            foreach ($action_id as $id) {
                $data = array(
                    ...
                    'modifieddate' => $now,
                    'modifiedby' => $this->mw_session['user_id']);
                $this->common_model->update('parameter_label_mst', 'id', $id, $data);
            }
            ...
            foreach ($action_id as $id) {               
                $DeleteFlag=1;
                if ($DeleteFlag) {                    
                    $this->common_model->delete('parameter_label_mst', 'id', $id);                    
                    $SuccessFlag = true;
                } else {
                    $alert_type = 'error';
                    $message = "Sub Parameter cannot be deleted.";
    ...
    public function Check_subparameter() {
        $parameterlabel = $this->input->post('description', true);  
		$subparameter_id    = $this->input->post('subparameter_id', true);
        if ($parameterlabel != '') {
            echo $this->parameterlabel_model->Check_subparameter_exist($parameterlabel,$subparameter_id);
        }
    }           
}
