...
class Ai_cronjob_model extends CI_Model {
    ...
    public function get_completed_video(){
        $query  = "SELECT * from ai_schedule WHERE  task_status=1 AND xls_generated=1 AND xls_imported=1 and (schedule_by=0 or schedule_by='' or isnull(schedule_by)) ORDER BY id";
        ...
        if($assessment_type == 3){
            $query = "SELECT 
                ...
                FROM( SELECT DISTINCT 
                    ...
                    WHERE ar.company_id = ".$company_id." AND ar.assessment_id = ".$assessment_id." AND tt.id > 0 AND tcs.question_id > 0 
                        AND ar.ftp_status=1 AND ar.vimeo_uri !='' AND ar.bucket_status = 2 
                        AND aa.is_completed =1 GROUP BY ar.user_id ORDER BY ar.user_id ) AS main 
            ...
            $query  = "SELECT
                    ...
                    FROM(
                        ...
                        WHERE arc.company_id = '".$company_id."' AND arc.assessment_id='".$assessment_id."'  AND arc.trans_id > 0 AND arc.question_id > 0 AND arc.ftp_status=1 AND arc.vimeo_uri !=''
                        GROUP BY arc.user_id ORDER BY arc.user_id) as ucj 
    ...
    // public function get_assessment_users($company_id,$assessment_id){
    //     $query  = "SELECT
    //                 main.*,@dcp AS previous,
    //                 CONCAT('Q',CONVERT (( SELECT CASE WHEN main.user_id = previous THEN @cnt := @cnt + 1 ELSE @cnt := 1 END ),UNSIGNED INTEGER)) AS question_series,
    //                 @dcp := main.user_id AS current,
    ...
    //                 ) AS main
    //                 CROSS JOIN ( SELECT @cnt := 0, @dcp := 0) AS qcounter 
    //                 WHERE user_id NOT IN (SELECT user_id FROM  ai_schedule where company_id = '".$company_id."' AND assessment_id = '".$assessment_id."' )
    //             ORDER BY
    //                main.user_id, main.trans_id";
    //     $result = $this->db->query($query);
    //     return $result->result();
