...
class Ai_process_model extends CI_Model
{
    ...
    {
        $query = "SELECT distinct am.id as assessment_id, CONCAT('[', am.id,'] ', am.assessment, ' - [', art.description, ']') as assessment, if(DATE_FORMAT(am.end_dttm,'%Y-%m-%d %H:%i')>=CURDATE(),'Live','Expired') AS status,am.assessment_type 
                FROM assessment_mst am 
                LEFT JOIN assessment_report_type as art on art.id=am.report_type
				WHERE am.status = 1
                GROUP BY am.id ORDER BY am.id DESC";
        ...
        $query = "SELECT
                ...
                FROM(
                    ...
                    FROM
                        ...
                        LEFT JOIN assessment_question as aq on ar.question_id=aq.id
                        LEFT JOIN assessment_attempts AS aa ON ar.assessment_id = aa.assessment_id AND ar.user_id = aa.user_id 
                        LEFT JOIN ai_schedule as ais on ais.assessment_id = ar.assessment_id and ais.user_id = ar.user_id and ais.question_id = ar.question_id 
                    WHERE
    ...
    public function get_participants_final_report_ai_reports($assessment_id1)
    {
    ...
    public function get_parameters_report($company_id, $assessment_id)
    {
    ...
    {
        $query = "SELECT ROUND( IF(ps.weighted_percentage=0, SUM(ps.percentage)/count(*), SUM(ps.weighted_percentage)/count(ps.question_id) ) ,2) AS score  FROM assessment_results_trans as ps 
        WHERE 
    ...
    public function get_user_overall_score_combined($company_id, $assessment_id, $user_id)
    {
        $query = "SELECT FORMAT(sum(comb.overall_score)/count(comb.overall_score),2) as overall_score from (SELECT * FROM (
		    ...
		    @dcp := ROUND(main.overall_score,2) AS current
            FROM (
            SELECT ps.user_id,ROUND( IF(ps.weighted_percentage=0, SUM(ps.percentage)/count(ps.question_id), SUM(ps.weighted_percentage)) ,2) AS overall_score  
            FROM assessment_results_trans AS ps 
    ...
    {
        $query = "SELECT ROUND( IF(ps.weighted_percentage=0, SUM(ps.percentage)/count(ps.question_id), SUM(ps.weighted_percentage)/count(ps.question_id)) ,2) AS score  FROM assessment_results_trans as ps 
        WHERE 
        ps.assessment_id = '" . $assessment_id . "' AND ps.user_id ='" . $user_id . "' AND ps.question_id ='" . $question_id . "'
        GROUP BY ps.user_id ,ps.question_id";
        $result = $this->db->query($query);
    ...
    public function get_parameters_reports($company_id, $assessment_id)
    {
    ...
    //Added for parameter labels for trinity and role-play - 17-01-24 changes by Anurag
    ...
    {
        $query = "SELECT  CONCAT(cu.first_name,' ', cu.last_name) as trainer_name, cu.username as trainer_no FROM ";
    ...
    {
        // $query = "SELECT DISTINCT IF (pl.id=0, ps.description, pl.description) as parameter_name, ps.id as parameter_id, pl.id as parameter_label_id,round(sum(ass.score)/count(*),2) as score
    ...
    //Added for parameter labels for trinity and role-play - 18-01-24 changes by Anurag
    ...
    // Manual parameter 05-04-2023
    public function get_manaul_parameters_reports($company_id, $assessment_id)
    {
        $query = "SELECT DISTINCT IF (pl.id=0 OR pl.id IS NULL, ps.description, pl.description) as parameter_name, ps.id as parameter_id, IF (pl.id=0 OR pl.id IS NULL,ps.id,pl.id) as parameter_label_id FROM parameter_mst as ps 
            LEFT JOIN parameter_label_mst as pl on pl.parameter_id= ps.id
            WHERE pl.id IN (SELECT parameter_label_id FROM `assessment_results_trans` where assessment_id='" . $assessment_id . "')
            ORDER BY  ps.id, pl.id";
        $result = $this->db->query($query);
        return $result->result();
    }
    ...
    public function get_manual_parameter_score($company_id, $assessment_id)
    {
        $query = "SELECT ps.user_id,IF (pl.id=0 OR pl.id IS NULL, p.description, pl.description) as parameter_name,
                  ...
                  LEFT JOIN parameter_label_mst as pl ON ps.parameter_label_id = pl.id AND ps.parameter_id = pl.parameter_id 
                  LEFT JOIN assessment_trans_sparam ats ON ats.parameter_id=ps.parameter_id AND ats.assessment_id=ps.assessment_id AND ats.question_id=ps.question_id 
                  WHERE ps.assessment_id = '" . $assessment_id . "' and p.company_id = '" . $company_id . "'
                  GROUP BY ps.user_id,ps.parameter_id,ps.parameter_label_id 
                  ORDER BY ps.user_id,ps.parameter_id,ps.parameter_label_id";
    ...
    public function get_manual_parameter_score_user($company_id, $assessment_id, $user_id)
    {
        $query = "SELECT ps.user_id,IF (pl.id=0 OR pl.id IS NULL, p.description, pl.description) as parameter_name,
                  p.id as parameter_id, IF (pl.id=0 OR pl.id IS NULL,p.id, pl.id) as parameter_label_id,
                  ROUND( IF(ps.weighted_percentage=0, SUM(ps.percentage)/count(*), 
...
// echo $query;exit;
    ...
    {
        $query = "SELECT tut.user_id, CONCAT(du.firstname,' ',du.lastname) as user, du.email,tut.attempts,IF(tut.speaker='T', 'Trinity', 'Rep') as speaker,tut.content,DATE_FORMAT(tut.addeddate,'%d-%m-%Y %h:%i:%s') as addeddate 
