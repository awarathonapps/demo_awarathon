...
class Assessment_dashboard_model extends CI_Model {
    ...
    public function get_Total_Assessment($Company_id, $start_date = '', $end_date = '', $region_id = '', $store_id = '', $supervisor_id, $report_type) {
        ...
        if($report_type == "2"){    
            ...
            $query = "SELECT count(total) as total_assessment 
            FROM (SELECT count(total_assessment) as total 
    ...
    public function get_region_average($Company_id, $report_by, $region_id = '', $SDate = '', $EDate = '', $assessment_id = '', $report_type,  $store_id = '') {
        $cond="";$cond1="";$cond2='';
        if ($SDate != '' && $EDate != '') {
        ...
        if($report_type=="2"){
            ...
            du.region_id, ";
                ...
                LEFT JOIN assessment_mst am ON am.id=art.assessment_id LEFT JOIN device_users du ON du.user_id=art.user_id 
    ...
    public function get_region_result($Company_id, $report_by, $region_id = '', $SDate = '', $EDate = '', $assessment_id = '', $store_id = '', $report_type, $supervisor_id) {
        $cond = '';$cond1 ='';$assmng="";
        if ($SDate != '' && $EDate != '') {
            $cond .=" AND (date(am.start_dttm) BETWEEN '" . $SDate . "' AND '" . $EDate . "'";
            $cond .=" OR date(am.end_dttm) BETWEEN '" . $SDate . "' AND '" . $EDate . "'";
    ...
    public function get_questions_score_new($dtWhere, $dtOrder, $dtLimit, $report_type) {
        if($report_type=="2"){
            ...
            $query = " SELECT  IF(am.ratingstyle=2, CONCAT(IFNULL(FORMAT(SUM(art.score)/ COUNT(pm.id),2),0),'%'), CONCAT(IFNULL(FORMAT(AVG(art.score),2),0),'%')) AS result,du.region_id,
                    ...
                    INNER JOIN device_users du ON du.user_id=art.user_id
                    LEFT JOIN assessment_question aq ON aq.id=art.question_id
                    $dtWhere AND parameter_type='parameter' AND art.question_id !=''";
            $query .= " group by art.question_id ";
            $query_count = $query;
            $query .= " $dtOrder $dtLimit ";
            $result = $this->db->query($query);
            ...
            $query = " SELECT FORMAT(SUM(main.result)/count(main.result),2) as result, main.question, main.question_id from( 
                ...
                ( SELECT  CONCAT(IFNULL(FORMAT(AVG(ar.score),2),0),'%') AS result,du.region_id,
                    ...
                    $dtWhere AND parameter_type='parameter' AND ar.question_id !='' group by ar.question_id ORDER BY ar.question_id asc)) as main
        ...
        if($report_type=="2"){
            $query = " SELECT a.*,ctr.range_color
                    FROM (
                        ...
                        FROM assessment_trainer_weights art 
                        -- LEFT JOIN ai_subparameter_score ais ON ais.assessment_id = art.assessment_id AND ais.parameter_id=art.parameter_id
                        ...
                        LEFT JOIN region rg ON rg.id=du.region_id 
                        WHERE art.assessment_id=$assessment_id AND du.region_id=$region_id
                        -- AND ais.parameter_type='parameter'
    ...
    public function get_parameter_assessment_result($company_id, $assessment_id ,$report_type) {
        if($report_type=="2")
        {
        $query = " SELECT a.*,ctr.range_color
                FROM (
    ...
    public function region_level_result($region_id, $StartDate = '', $EndDate = '', $pass_range_from, $pass_range_to, $fail_range_from, $fail_range_to, $report_type) {
        $cond = '';
        if ($StartDate != '' && $EndDate != '') {
            $cond .=" AND (DATE(am.start_dttm) BETWEEN '" . $StartDate . "' AND '" . $EndDate . "'";
            $cond .=" OR DATE(am.end_dttm) BETWEEN '" . $StartDate . "' AND '" . $EndDate . "'";
    ...
    public function assessment_index_weekly_monthly($report_by,$StartDate = '', $EndDate = '', $report_type,$user_id = '') {
        $cond="";
    ...
    public function assessment_index_yearly($report_by,$StartDate = '', $EndDate = '', $report_type) {
        $cond = '';
        $ResultArray = array();$PeriodArray = array();$AssessArray = array();
        if ($StartDate != '' && $EndDate != '') {
            $cond .=" AND (date(am.start_dttm) BETWEEN '" . $StartDate . "' AND '" . $EndDate . "'";
