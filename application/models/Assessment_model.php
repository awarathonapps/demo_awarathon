...
//  }
    ...
    public function get_user_trainer_data($assessment_id, $user_id)
    {
        $query = "SELECT u.userid as trainer_id,CONCAT(u.first_name,' ',u.last_name) as name 
				FROM company_users u LEFT JOIN assessment_mapping_user amu on u.userid=amu.trainer_id 
				WHERE amu.user_id='$user_id'";
        $query = $this->db->query($query);
        return $query->result();
    }
