...
class Parameterlabel_model extends CI_Model {
    public function __construct() {
        parent::__construct();
    }    
    public function LoadDataTable($dtWhere, $dtOrder, $dtLimit) {
        $query = " SELECT p.*,pm.description as parameter_name FROM parameter_label_mst as p ";
        ...
        $query_count = "SELECT COUNT(p.id) as total FROM parameter_label_mst as p ";
        $query_count .= " $dtWhere";
    ...
    public function Check_subparameter_exist($subparameter,$subparameter_id) {        
        $querystr="select description from parameter_label_mst where description='" . $subparameter . "'";
		if($subparameter_id!=''){
