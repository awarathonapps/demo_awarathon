                    ...
                    <div class="page-content">
                        ...
                        <div class="page-bar">
                            <ul class="page-breadcrumb">
                                ...
                                <li>
                                    <span>Workshop Sub-Type</span>
                                    <i class="fa fa-circle"></i>
                                </li>
                                <li>
                                    <span>Workshop Subtype Listing</span>
                                </li>
                            </ul>
                            <?php if($acces_management->allow_add){ ?>
                            <div class="page-toolbar">
                                <a class="btn btn-sm btn-orange pull-right" id="btnaddpanel3"  href="<?php echo base_url() . 'workshopsubtype/LoadModal/' ?>" 
                                                            ...
                                                            <div class="col-md-6">
                                                                <div class="form-group">
                                                                    ...
                                                                    <div class="col-md-9" style="padding:0px;">
                                                                        <select id="filter_cmp_id" name="filter_cmp_id" class="form-control input-sm select2me filter_list" placeholder="Please select"  style="width: 100%" onchange="getFilterdWorkshopType();">
                                                 ...
                                                 <?php } ?>  
                                                    ...
                                                    <div class="col-md-6">       
                                                            <div class="form-group">
                                                                ...
                                                                <div class="col-md-9" style="padding:0px;">
                                                                <select id="filter_wtype_id" name="filter_wtype_id" class="form-control input-sm select2me filter_list" placeholder="Please select">
                                                                    <option value="">Please Select</option>
                                                                    <?php if(count($WorkshopType)>0){
                                                                            foreach ($WorkshopType as $Row) { ?>
                                                                                <option value="<?php echo $Row->id ?>"><?php echo $Row->workshop_type ?></option>
                                                                        <?php  }
                                                                    } ?>
                                                                </select>
                                                            </div></div>
                                                    ...
                                                    <div class="col-md-6">    
                                                            <div class="form-group">
                                                                ...
                                                                <div class="col-md-9" style="padding:0px;">
                                                                <select id="filter_status" name="filter_status" class="form-control input-sm select2me filter_list" placeholder="Please select" >
                                                                    <option value="">Please Select</option>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="portlet light bordered">
                                    ...
                                    <div class="portlet-body">
                                        <form id="frmWsubtype" name="frmWsubtype" method="post">
...
<script src="<?php echo $asset_url; ?>assets/global/plugins/datatables/Buttons-1.3.1/js/dataTables.buttons.min.js" type="text/javascript"></script>
        <script>
            var wsubtype_edit_id='';
            var workshop_type_id='';            
            ...
            function resetDATA(){
                //$('#workshop_type_id,#company_id').val(null).trigger('change');
                $('.select2me').select("val", "");
                $('.select2me').val(null).trigger('change');
                document.FilterFrm.reset();
            }
            function getComapnywiseWorkshopType(){
                   $.ajax({
                    type: "POST",
                    data: "data=" + $('#company_id').val(),
                    async:false,
                    url: "<?php echo base_url(); ?>workshopsubtype/ajax_company_wtype",
                    beforeSend: function () {
                    	customBlockUI();
                    },
                        ...
                        if (msg != '') {
                            ...
                            var WTypeMSt = Oresult['result'];                           
                            var option='<option value="">Please Select</option>';
                            for (var i = 0; i < WTypeMSt.length; i++) {
                                option += '<option value="' + WTypeMSt[i]['id'] + '" '+(workshop_type_id==WTypeMSt[i]['id'] ? 'Selected':'')+'  >' + WTypeMSt[i]['workshop_type'] + '</option>';
                            }
                            $('#workshop_type_id').empty();
                            $('#workshop_type_id').append(option);
                            //$("#workshop_type_id").trigger("change");
                        }
                        customunBlockUI();
                    }
                });
            }
            function getFilterdWorkshopType(){
                if($('#filter_cmp_id').val()==""){
                     $('#filter_wtype_id').empty();
                    return false;
                }
                   $.ajax({
                    type: "POST",
                    ...
                    success: function (msg) {
                        if (msg != '') {
                            ...
                            for (var i = 0; i < WTypeMSt.length; i++) {
                                option += '<option value="' + WTypeMSt[i]['id'] + '">' + WTypeMSt[i]['workshop_type'] + '</option>';
                            }
                            $('#filter_wtype_id').empty();
                            $('#filter_wtype_id').append(option);
...
//                        if (response.message==''){     
//                            wsubtype_edit_id=response.result[0].id;
//                            workshop_type_id =response.result[0].topic_id;
...
//            }
                ...
                $.confirm({
                    ...
                    buttons: {
                        ...
                        action: function(){
                            $.ajax({
                                type: "POST",
                                data: $('#frmWsubtype').serialize(),
                                url: "<?php echo $base_url; ?>workshopsubtype/record_actions/"+opt,
                                beforeSend: function () {
                                    customBlockUI();
                                },
                                success: function (response_json) {
                ...
                if (Check > 0){
                    if( opt == 1 ){
                       LoadConfirmDialog("Confirm Active Status ?",opt);  
                        ...
                        LoadConfirmDialog("Delete Selected Workshop Subtype(s) ?",opt);
                    }   
                }else{
                ...
                $.confirm({
                    ...
                    buttons: {
                        ...
                        action: function(){
                            $.ajax({
                                type: "POST",
                                url: "<?php echo $base_url;?>workshopsubtype/remove",
                                data: {deleteid:Id},
                                beforeSend: function () {
                                    customBlockUI();
                                },
            ...
            {
                var x = 0;
                for (var i = 0; i < frmWsubtype.elements.length; i++)
                {
                    if (frmWsubtype.elements[i].checked == true)
                    {
                        x++;
                    }
                }
...
//                }
                ...
                table.dataTable({
                    ...
                    "processing": true,
                    "serverSide": true,
                    "sAjaxSource": "<?php echo base_url() . 'workshopsubtype/DatatableRefresh'; ?>",
                    "fnServerData": function (sSource, aoData, fnCallback) {
                        aoData.push({name: '__mode', value: 'featuredimage.ajaxload'});
                        aoData.push({name: 'filter_wtype_id', value: $('#filter_wtype_id').val()});
