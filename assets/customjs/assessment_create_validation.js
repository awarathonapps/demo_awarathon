...
function getparameter(Q_id, srno, cnt) {
    $.ajax({
        ...
        type: 'POST',
        data: {
            company_id: $('#company_id').val(), ass_result_id: $('#ass_result_id').val(),
...
function UpdateAssessment() {
    ...
    if (assessment_type == "2") {
        ...
        // total_question = (+selected_question) + (+old_question_total);
    ...
    if ($("#assessment_type").prop("disabled") == true) {
        form_data['assessment_type'] = assessment_type;
    }
    //KRISHNA -- Spotlight changes end
    var x = 0;
    var sub_parameter = [];
...
function DatatableUsersRefresh() {
    ...
    oTable = table.dataTable({
        ...
        "columnDefs": [
            ...
            { 'width': '80px', 'orderable': true, 'searchable': false, 'targets': [6] }
        ],
        "order": [
            [1, "desc"]
        ],
...
function confirm_allow_users() {
    ...
    $.confirm({
       title: 'Confirm!',
       content: "Are you sure you want to send email notification?",
       buttons: {
          confirm: {
             text: 'Yes',
             btnClass: 'btn-primary',
...
function ConfirmUsers(flag = '') {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { NewUsersArrray: NewUsersArrray,flag:flag},
        beforeSend: function () {
            customBlockUI();
        },
        success: function (Odata) {
...
function confirm_allow_managers() {
...
function ConfirmManagers(flag = '') {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { NewManagersArrray: NewManagersArrray,flag:flag },
        beforeSend: function () {
            customBlockUI();
        },
        success: function (Odata) {
...
function LoadFilterUserData(is_mapped) {
    $('#UserFilterTable').dataTable({
        ...
        "columnDefs": [
            ...
            { 'width': '80px', 'orderable': true, 'searchable': true, 'targets': [7] }
        ],
        "order": [
            [0, "asc"]
        ],
        ...
        "fnServerData": function (sSource, aoData, fnCallback) {
            ...
            aoData.push({ name: 'flt_department_id', value: $('#flt_department_id').val() });
...
function selected_questions(id) {
    if ($('#ck_Question_id' + id).prop('checked')) {
        ...
        NewQuestionArray.splice($.inArray(id, NewQuestionArray), 1);
    }
}
        ...
        $("input[name='chksp_list[]']").each(function (index) {
            ...
            if ($('#chksp_id' + temp_id).prop('checked')) {
                ...
                if ($('#type_id' + temp_id).css('display') == 'none' || $('#type_id' + temp_id).css("visibility") == "hidden") {
                    ...
                    /*var format = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/;
                    if(format.test(sentence_keyword)){
...
function Confirm_questions(edit_id) {
    ...
    var active_language = $('.language_id').val();
    if (edit_id != '') {
        ...
        var tdata = { question_id: new_question_id, active_language: active_language };
    } else {
        ...
        tdata = { NewQuestionArray: NewQuestionArray, Encode_id: Encode_id, assessment_type: assessment_type, start_date: start_date, active_language: active_language };
    }
    $.ajax({
        url: turl,
        type: 'POST',
            ...
            if (Data['lchtml'] != '') {
                if (edit_id != "") {
                    ...
                    if (Data['Msg'] == '') {
                        ...
                        $('#VQA_refDatatable').append(Data['ref_html']);
                        $(".select2").select2();
                        $('#LoadModalFilter').data('modal', null);
                        ...
                        $('.language_id').on('select2:select', function (e) {
                            ...
                            $(".txt_trno").each(function () {
                                ...
                                let temp_language_id = ".language_id";
...
function UploadXlsUser_Manager() {
    ...
    $.ajax({
        ...
        processData: false,
        type: "POST",
        url: Base_url + 'assessment_create/confirm_upload_user_manager/' + Encode_id,
        data: form_data,
        beforeSend: function () {
            customBlockUI();
        },
...
function SaveAssessment(mode) {
    ...
    //KRISHNA -- Spotlight changes start
    if ($("#assessment_type").prop("disabled") == true) {
        form_data['assessment_type'] = $('#assessment_type').val();
    }
    //KRISHNA -- Spotlight changes end
    var x = 0;
    var sub_parameter = [];
...
function refrence_video_table(que_id = '', tr_no = '') {
    $.ajax({
        url: Base_url + "assessment_create/append_ref_video_data/",
        type: 'POST',
        data: { tr_no: tr_no, que_id: que_id },
        beforeSend: function () {
            customBlockUI();
        },
        success: function (Odata) {
            ...
            if (Data != '') {
                // console.log(tr_no);
                $("#LoadModalRef_video").modal('hide');
                var div_element = "#video_url" + tr_no;
                $(div_element).empty();
                $(div_element).text('');
                $(div_element).text(Data['video_url']);

                var file_preview = "#file_preview" + tr_no;
                $(file_preview).html(Data['tab']);
            }
            customunBlockUI();
        }
    });
}
function preview_video(video_url) {
    $.ajax({
        url: Base_url + "assessment_create/preview_video/",
        type: 'POST',
        ...
        success: function (Odata) {
            var Data = $.parseJSON(Odata);
            if (Data != '') {
                $("#LoadModalVideo").modal('show');
                $('#video_url_append').attr('src', Data['video_url'])
            }
            customunBlockUI();
        }
    });
...
function ref_video_remove(que_id) {
    $.ajax({
        url: Base_url + "assessment_create/remove_ref_video/" + Encode_id,
        type: 'POST',
        data: { que_id: que_id },
        beforeSend: function () {
            customBlockUI();
...
// KRISHNA -- Spotlight changes start  - Assesment type button disabled
assessment_type_change();
function assessment_type_change() {
    var assessment_type = $('#assessment_type').val();
    if (assessment_type == "") {
        $('#otc').prop('disabled', true);
        $('#assessment_name').prop('disabled', true);
        ...
        $('#q_limit').hide();
        $('#IsPreview').hide();
        $('#number_attempts').prop('disabled', true);
        $('#ratingstyle').prop('disabled', true);
        // to hide description and instruction
        $('#text_area').hide();
        // to hide description and instruction

        $('#P_table').hide();
        $('#Ref_table').hide();
        $('#Ref_video_rights').hide();
        $("#VQADatatable tr:first").find('#label_dyamic2').hide();
        $("#VQA_refDatatable tr:first").find('#label_dyamic_ref2').hide();

        // $('#assesment_type_1').hide();
        $('#q_type').hide();
        $('#btnaddpanel3').attr("disabled", true);




    }
    else if (assessment_type != "" && assessment_type == "1") {
        $('#assessment_type').prop('disabled', true);
        $('#otc').prop('disabled', false);
        $('#assessment_name').prop('disabled', false);
        $('#report_type').prop('disabled', false);
        $('#start_date').prop('disabled', false);
        $('#st_date').prop('disabled', false);

        $('#end_date').prop('disabled', false);
        $('#ed_date').prop('disabled', false);

        // for roleplay hide fileds
        $('#q_limit').hide();
        $('#IsPreview').hide();
        $('#text_area').show();

        // for roleplay hide fields
        $('#number_attempts').prop('disabled', false);
        $('#ratingstyle').prop('disabled', false);
        // $('#VQADatatable').prop('disabled', false);
        $('#P_table').show();
        $('#Ref_table').show();
        $('#Ref_video_rights').show();


        $("#VQADatatable tr:first").find('#label_dyamic2').hide();
        $("#VQA_refDatatable tr:first").find('#label_dyamic_ref2').hide();
        $('#q_type').hide();
        $('#btnaddpanel3').attr("disabled", false);
        // for title mapping quetion 
        $('#title').append("Mapping Questions:");
        $('#refrence_title').text("Mapping of a Reference Video: (Optional)");
        $('#refrence_video_reights').text("Reference Video Access:");

    } else if (assessment_type != "" && assessment_type == "2") {
        $('#assessment_type').prop('disabled', true);
        $('#otc').prop('disabled', false);
        $('#assessment_name').prop('disabled', false);
        $('#report_type').prop('disabled', false);
        ...
        $('#Ref_table').show();
        $('#Ref_video_rights').show();
        $("#VQADatatable tr:first").find('#label_dyamic2').show();

        // $('#assesment_type_2').hide();
        // $('#assesment_type_1').show();
        $('#q_type').show();
        $('#btnaddpanel3').attr("disabled", false);
        // for title mapping quetion 
        $('#title').append("Mapping Questions/Situation:");
        $('#refrence_title').text("Mapping of a Reference Video:");
    }
}
//KRISHNA -- Spotlight changes end
function video_modal(tr_no, question_id) {
    if (question_id == '' || question_id == null) {
        ShowAlret("Question id required", 'error');
        return false;
    }

    $.ajax({
        url: Base_url + "assessment_create/edit_refrence_video",
        type: 'POST',
        data: { tr_no: tr_no, question_id: question_id, assessment_id: Encode_id },
        beforeSend: function () {
            customBlockUI();
        },
        success: function (dthtml) {
            // $('#LoadModalRef_video').modal('show');
            $('#refv-modal-body').html(dthtml);
            $('#LoadModalRef_video').modal();
            customunBlockUI();
        }
    });
}